...
# Plot for Age vs Anxiety Levels
plt.figure(figsize=(10,5))
sns.lmplot(x="Age", y="Anxiety Level (1-10)", data=df, hue="Gender", aspect=1.5, scatter_kws={'alpha':0.3})
plt.title("Age vs. Anxiety Level (with Gender)")
plt.tight_layout()
plt.show()

# Count plot for gender-wise anxiety distribution
plt.figure(figsize=(8,5))
...
# Work stress impact
plt.figure(figsize=(8,5))
sns.histplot(data=df, x="Stress Level (1-10)", hue="Anxiety Level (1-10)", multiple="stack", palette="rocket")
plt.title("Anxiety Distribution Across Stress Levels")
plt.tight_layout()
plt.show()
...
plt.title("Impact of Major Life Events on Anxiety")
plt.show()
...
sns.heatmap(numeric_df.corr(), annot=True, cmap="coolwarm", linewidths=0.5, center=0)
...
# Top correlated features with anxiety
print(numeric_df.corr()["Anxiety Level (1-10)"].sort_values(ascending=False))
